...
export const Utils = {
	 ...
	 * @category Utils.pipe
	 */
	pipe: (...fns: Fn<any, any>[]): Fn<any, any> => {
		if (fns.length === 0) {
		...
		return fns.reduce((acc, fn) => (input: any) => fn(acc(input))) as Fn<any, any>;
	},
	 ...
	 */
	compose: (...fns: Fn<any, any>[]): Fn<any, any> => {
		if (fns.length === 0) {
		...
		return fns.reduceRight((acc, fn) => (input: any) => fn(acc(input))) as Fn<any, any>;
	},
